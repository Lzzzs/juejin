        ...
        <div class="detail">
          <div class="detail__header">
            <div class="detail__userInfo">
              <nuxt-link :to="'/user/' + articDetail.attributes.author.data.id" class="detail__user__avatar" target="_blank">
                <user-avatar :url="articDetail.attributes.author.data.attributes.avatar_large.data.attributes.url" :round="true"></user-avatar>
              </nuxt-link>
              <nuxt-link :to="'/user/' + articDetail.attributes.author.data.id" class="detail__user__name" target="_blank">
                <span style="margin-right: 10px">{{ articDetail.attributes.author.data.attributes.user_name }}</span>
                <level :level="articDetail.attributes.author.data.attributes.level"></level>
              </nuxt-link>
              <div class="detail__user__meta">
                <span class="meta__time">{{ articDetail.attributes.ctime | formatTime }}</span>
...
<!--            <follow-btn :is-follow.sync="articDetail.attributes.is_follow" type="user" :followee-id="articDetail.attributes.author.data.id"></follow-btn>-->
          ...
          <div v-if="articDetail.attributes.cover_image.data" class="detail__cover" :style="`background-image: url(${articDetail.attributes.cover_image.data.attributes.url})`"></div>
    ...
    <div class="detail-side">
      ...
      <about-article :list="aboutArticles.attributes.articles.data"></about-article>
      <catalog></catalog>
    </div>
...
export default {
  async asyncData ({ app, params }) {
    ...
    aboutArticles = await app.$api.getRelatedEntry({
      tag_id: articDetail.attributes.header_tags.data.map(item => item.id)[0]
    }).then(res => res.data)
    console.log("_id.vue里面的aboutArticles")
    console.log(aboutArticles)
    console.log('\n')
